...
export async function validateCredentialLogin(username: string, password: string, hwid: string) {
  ...
  if (!license || !license.passwordHash || !license.passwordSalt) {
...
export async function enableLicense(key: string) {
  ...
  const now = new Date();
  const status =
    license.expiresAt && license.expiresAt.getTime() <= now.getTime() ? LicenseStatus.EXPIRED : LicenseStatus.ACTIVE;
...
async function getInactiveResult(license: License | null, client: LicenseClient = prisma) {
  ...
  const now = new Date();
  if (license.status === LicenseStatus.EXPIRED || (license.expiresAt && license.expiresAt.getTime() <= now.getTime())) {
...
async function expireLiteTrials(client: LicenseClient, now: Date) {
  ...
  await client.liteTrial.updateMany({
    where: { id: { in: expiredTrials.map((trial) => trial.id) } },
    data: { status: LicenseStatus.EXPIRED },
  });

  await client.license.updateMany({
    where: { id: { in: expiredTrials.map((trial) => trial.licenseId) } },
    data: { status: LicenseStatus.EXPIRED },
  });
}
